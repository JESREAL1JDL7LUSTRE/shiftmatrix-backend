...
export const clockInEndpoint: Endpoint = {
  ...
  handler: async (req) => {
    ...
    // Fetch shift + associated department
    ...
    // QR Code Validation (Proof of Presence)
    if (!department?.currentDailyToken || department.currentDailyToken !== qrToken) {
      return Response.json(
        { error: 'Invalid or expired QR Token. Please scan the current token generated by the Admin.' },
        { status: 403 }
      )
    }
